...
# below
        ...
        t_elapsed, t_all = utils.time_text(t), utils.time_text(t / (epoch - epoch_start + 1))
...
def save(config, model, save_path, name):
    if config['model']['name'] == 'segformer' or config['model']['name'] == 'setr':
        ...
        torch.save(model.state_dict(), os.path.join(save_path, f"model_epoch_{name}.pth"))
    # Save the entire model for every epoch
    torch.save(model, os.path.join(save_path, f"model_epoch_{name}.pt"))
